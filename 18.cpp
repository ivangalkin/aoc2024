...
ll get_shortest_path(ll SIZE, ll LIMIT) {
  const size_t vertex = SIZE * SIZE;
  auto *adj = new std::vector<pair<int, int>>[vertex];

  auto ID = [SIZE](ll x, ll y) { return y*SIZE + x;};

  auto CORRUPTED = [SIZE, LIMIT] (ll x, ll y) {
    auto end = std::next(input.begin(), LIMIT);
    return std::find(input.begin(), end, std::make_tuple(x, y)) != end;
  };
  ...
  for (ll y = 0; y < SIZE; y++) {
    for (ll x = 0; x < SIZE; x++) {
        ...
        for (auto [dx, dy]: DIFFs) {
            ...
            if (!CORRUPTED(nx, ny)) {
                adj[ID(x, y)].push_back(std::make_pair(ID(nx, ny), 1));
            }
        }
